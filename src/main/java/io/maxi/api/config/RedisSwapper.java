...
import io.maxi.api.utils.KryoUtil;
...
@Component
public class RedisSwapper {

    protected static final org.slf4j.Logger log = LoggerFactory.getLogger(RedisSwapper.class);

    @Autowired
    private JedisPool jedisPool;

    /**
     * 超过一整天 24+8都没有刷新过的数据,可以被剔除
     */
    public static final int memcahce_expire_seconds = 28*60*60;

    private final LoadingCache<String, Optional<byte[]>> cache = Caffeine.newBuilder()
            .expireAfterWrite(memcahce_expire_seconds, TimeUnit.SECONDS)
            .expireAfterAccess(memcahce_expire_seconds,TimeUnit.SECONDS)
            .softValues()
            .initialCapacity(512)
            .maximumSize(4096)
            .removalListener(((key, value, cause) -> {
                log.info("[RedisSwapper][cache] remove  key:{}  value:{}  cause:{}",key,value,cause);
            }))
            .build(key -> {
                log.info("[RedisSwapper][cache] CacheLoader  key:{} ",key);
                byte[] value = get(key.getBytes());
                return Optional.ofNullable(value);
            });
    private final LoadingCache<String, Optional<Object>> objectCache = Caffeine.newBuilder()
            .expireAfterWrite(memcahce_expire_seconds, TimeUnit.SECONDS)
            .expireAfterAccess(memcahce_expire_seconds,TimeUnit.SECONDS)
            .softValues()
            .initialCapacity(512)
            .maximumSize(4096)
            .removalListener(((key, value, cause) -> {
                log.info("[RedisSwapper][objectCache] remove  key:{}  value:{}  cause:{}",key,value,cause);
            }))
            .build(key -> {
                log.info("[RedisSwapper][objectCache] CacheLoader  key:{} ",key);

                return Optional.ofNullable(null);
            });
    /******************************************************************************************************************************************/
    /******************************************************************************************************************************************/
    /**************************************特别注意,内存缓存,只能存放一些极少变化的数据,且需要自行维护数据失效刷新***************************************/
    /**************************************特别注意,内存缓存,只能存放一些极少变化的数据,且需要自行维护数据失效刷新***************************************/
    /**************************************特别注意,内存缓存,只能存放一些极少变化的数据,且需要自行维护数据失效刷新***************************************/
    /**************************************特别注意,内存缓存,只能存放一些极少变化的数据,且需要自行维护数据失效刷新***************************************/
    /**************************************特别注意,内存缓存,只能存放一些极少变化的数据,且需要自行维护数据失效刷新***************************************/
    ...
    /******************************************************************************************************************************************/
    /**
     * 优先从内存中获取一个对象,如果没有找到,就去redis中找
     * @param key
     * @param clazz
     * @param <T>
     * @return
     */
    public <T> T memGet(String key, Class<T> clazz) {

        Optional<Object> objOpt = objectCache.get(key);
        if(objOpt.isPresent()){
            return (T) objOpt.get();
        }

        Optional<byte[]> opt = cache.get(key);
        if(opt.isPresent()){
            byte[] bytes = opt.get();
            memExpire(key);
            T obj = KryoUtil.readObjectFromByteArray(bytes,clazz);
            objectCache.put(key,Optional.ofNullable(obj));
            //对象缓存了,byte就清掉,节约内存
            cache.invalidate(key);
            return obj;
        }
        //null时候,直接返回,需要外面从源头获取
        return null;
    }
     ...
     * @return
     */
    public boolean memExpire(String key,int seconds){
        return expire(key.getBytes(),seconds) != 0;
    }
    public boolean memExpire(String key){
        return memExpire(key,memcahce_expire_seconds);
    }

    public <T> T memGet(String key, Class<T> clazz,CacheCallback<T> callback) {
        T result = memGet(key, clazz);
        if(result != null){
            return result;
        }
        //缓存中查询不到,就去源头获取,获取到了,刷新缓存,获取不到,下次再说
    ...
    public String memSet(String key,Object object,int seconds){
        if(object == null){
            return null;
        }
        //先序列化
        byte[] bytes = KryoUtil.writeObjectToByteArray(object);
        //先存redis
        String result = set(key.getBytes(),bytes);
        //设置过期时间
        expire(key,seconds);
        //清理本地缓存,下次重新加载
        cache.invalidate(key);
    ...
    /**
     * 将一个对象放入缓存中,并直接刷新到本地内存
     * @param key
     * @param object
     * @return
     */
    public String memSet(String key,Object object){
        return memSet(key, object,memcahce_expire_seconds);
    }
    ...
    public void cleanMen(String key){
        //清除本地
        cache.invalidate(key);
        objectCache.invalidate(key);
    }
